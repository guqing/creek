...
public class GlobalExceptionHandler {
    ...
    public ResultEntity<String> handleHttpMediaTypeNotSupportedException(
    ...
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    public ResultEntity<String> handleHttpRequestMethodNotSupportedException(
    ...
    public ResultEntity<String> handleBindSocialAccountException(BindSocialAccountException e) {
        log.error("绑定社交帐号出错:{}", e.getMessage(), e);
    ...
    /**
     * 当使用@Valid不带@RequestBody request参数时: 对象验证失败，验证将引发BindException而不是MethodArgumentNotValidException
     *
     * @param e 参数绑定异常
    ...
    /**
     * 使用@Valid并且带有@RequestBody request参数时 参数教研失败将抛出MethodArgumentNotValidException异常，由此方法捕获处理
     *
     * @param e 方法参数校验失败的异常
     * @return 返回校验失败错误信息
     */
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ResultEntity<Map<String, String>> validExceptionHandler(
